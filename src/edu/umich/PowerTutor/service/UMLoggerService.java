...
public class UMLoggerService extends Service {
  ...
  public void onDestroy() {
    ...
    if (!foregroundSet) {
      Toast.makeText(UMLoggerService.this, "Service could not be stopped from Foreground!", Toast.LENGTH_SHORT).show();
  ...
  public void showNotification() {
    ...
    /*
     * We need to set the service to run in the foreground so that system won't
     * try to destroy the power logging service except in the most critical
     * situations (which should be fairly rare). Due to differences in apis
     * across versions of android we have to use reflection. The newer api
     * simultaneously sets an app to be in the foreground while adding a
     * notification icon so services can't 'hide' in the foreground. In the new
     * api the old call, setForeground, does nothing. See:
     * http://developer.android
     * .com/reference/android/app/Service.html#startForeground
     * %28int,%20android.app.Notification%29
     */
    boolean foregroundSet = false;
    try {
    ...
    if (!foregroundSet) {
      // setForeground(true);
      Toast.makeText(UMLoggerService.this, "Service could not be set to Foreground!", Toast.LENGTH_SHORT).show();
      notificationManager.notify(NOTIFICATION_ID, notification);
    }
  }

  /*
   * This function is to update the notification in real time. This function is
   * apparently fairly expensive cpu wise. Updating once a second caused a 8%
  ...
  public void updateNotification(int level, double totalPower) {
    ...
    if (bst.hasCharge() && bst.hasVoltage()) {
      ...
      if (charge > 0 && volt > 0) {
        ...
        if (minutes < 55) {
          notification.iconLevel = 1 + (int) Math.max(0, Math.round(minutes / 10.0) - 1);
        } else {
          notification.iconLevel = (int) Math.min(13, 6 + Math.max(0, Math.round(minutes / 60.0) - 1));
        }
      }
    }

    CharSequence contentTitle = "PowerTutor";
    CharSequence contentText = "Total Power: " + (int) Math.round(totalPower) + " mW";
  ...
  BroadcastReceiver broadcastIntentReceiver = new BroadcastReceiver() {
    public void onReceive(Context context, Intent intent) {
      if (intent.getAction().equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
        ...
        powerEstimator.writeToLog("battery-change " + intent.getIntExtra("plugged", -1) + " "
            + intent.getIntExtra("level", -1) + "/" + intent.getIntExtra("scale", -1) + " "
            + intent.getIntExtra("voltage", -1) + intent.getIntExtra("temperature", -1) + "\n");
        ...
        SystemInfo.getInstance().voidUidCache(intent.getIntExtra(Intent.EXTRA_UID, -1));
      }
    };
  };
    ...
    public void onServiceStateChanged(ServiceState serviceState) {
      ...
      case ServiceState.STATE_IN_SERVICE:
        ...
        case (TelephonyManager.NETWORK_TYPE_UMTS):
          ...
          break;
        default:
          powerEstimator.writeToLog("phone-network " + phoneManager.getNetworkType() + "\n");
        }
        break;
