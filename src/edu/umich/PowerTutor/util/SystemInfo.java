...
public class SystemInfo {
  ...
  public static final int AID_ALL = -1; /*
                                         * A special constant we will use to
                                         * indicate a request for global
                                         * information.
                                         */
  ...
  public static final int AID_DIAG = 2002; /*
                                            ...
                                            */
  /*
   * The 3000 series are intended for use as supplemental group id's only. They
   * indicate special Android capabilities that the kernel is aware of.
   */
  public static final int AID_NET_BT_ADMIN = 3001; /*
                                                    * bluetooth: create any
                                                    * socket
                                                    */
  public static final int AID_NET_BT = 3002; /*
                                              * bluetooth: create sco, rfcomm or
                                              * l2cap sockets
                                              */
  public static final int AID_INET = 3003; /*
                                            * can create AF_INET and AF_INET6
                                            * sockets
                                            */
  ...
  private static final int[] PROCESS_STATS_FORMAT = new int[] { PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM,
      PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM,
      PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM, PROC_SPACE_TERM | PROC_OUT_LONG, // 13:
                                                                                                           // utime
      PROC_SPACE_TERM | PROC_OUT_LONG // 14: stime
  };
  private static final int[] PROCESS_TOTAL_STATS_FORMAT = new int[] { PROC_SPACE_TERM, PROC_SPACE_TERM | PROC_OUT_LONG,
      PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG,
      PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG, };
  private static final int[] PROC_MEMINFO_FORMAT = new int[] { PROC_SPACE_TERM | PROC_COMBINE,
      PROC_SPACE_TERM | PROC_OUT_LONG, PROC_LINE_TERM, PROC_SPACE_TERM | PROC_COMBINE, PROC_SPACE_TERM | PROC_OUT_LONG,
      PROC_LINE_TERM, PROC_SPACE_TERM | PROC_COMBINE, PROC_SPACE_TERM | PROC_OUT_LONG, PROC_LINE_TERM,
  ...
  private SystemInfo() {
    ...
    try {
      methodReadProcFile = Process.class.getMethod("readProcFile", String.class, int[].class, String[].class,
  ...
  /*
   * lastPids can be null. It is just used to avoid memory reallocation if at
   * all possible. Returns null on failure. If lastPids can hold the new pid
   * list the extra entries will be filled with -1 at the end.
   */
  ...
  /*
   * lastUids can be null. It is just used to avoid memory reallocation if at
   * all possible. Returns null on failure. If lastUids can hold the new uid
   * list the extra entries will be filled with -1 at the end.
   */
  public int[] getUids(int[] lastUids) {
    if (methodGetPids == null)
      return manualGetInts("/proc/uid_stat", lastUids);
    try {
  ...
  public boolean getPidUsrSysTime(int pid, long[] times) {
    if (methodReadProcFile == null)
      return false;
    try {
      return (Boolean) methodReadProcFile.invoke(null, "/proc/" + pid + "/stat", PROCESS_STATS_FORMAT, null, times,
          null);
    } catch (IllegalAccessException e) {
   ...
   * with the total user time, times[INDEX_SYS_TIME] will be filled with the
   * total sys time, and times[INDEX_TOTAL_TIME] will have the total time
   * (including idle cycles). Returns true on success.
   */
  public boolean getUsrSysTotalTime(long[] times) {
    if (methodReadProcFile == null)
      return false;
    try {
      if ((Boolean) methodReadProcFile.invoke(null, "/proc/stat", PROCESS_TOTAL_STATS_FORMAT, null, times, null)) {
  ...
  public boolean getMemInfo(long[] mem) {
    if (methodReadProcFile == null)
      return false;
    try {
      if ((Boolean) methodReadProcFile.invoke(null, "/proc/meminfo", PROC_MEMINFO_FORMAT, null, mem, null)) {
        return true;
      }
    } catch (IllegalAccessException e) {
  ...
  private static class UidCacheEntry {
    ...
    public void clearIfExpired() {
      if (updateTime != -1 && updateTime + EXPIRATION_TIME < SystemClock.elapsedRealtime()) {
        updateTime = -1;
        name = null;
        icon = null;
      }
