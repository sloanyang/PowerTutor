...
public interface PhoneConstants {
  ...
  /*
   * Gives the time in seconds that the GPS sleeps for after the session has
   * ended.
   */
  ...
  /*
   * Gives the packet rate needed to transition from the low power state to the
   * high power state.
   */
  public double wifiLowHighTransition();

  /*
   * Gives the packet rate needed to transition from the high power state to the
   * low power state.
   */
  public double wifiHighLowTransition();

  /*
   * Gives the power/uplinkrate for different link speeds for wifi in high power
   * mode.
   */
  public double[] wifiLinkRatios();

  /*
   * Gives the link speed associated with each link power ratio. Elements should
   * be in increasing order. Should have the same number of elements as
   * wifiLinkRatios().
   */
