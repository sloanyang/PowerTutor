...
public class DataSource implements Serializable {
  ...
  private static final String[] idShorts = { "Power", "Battery Time", "Charge", "Voltage", "Current", "Temperature",
      "Percent Battery", };
  private static final String[] idLongs = { "Displays power in mW",
      "Displays remaining battery lifetime.  Also can show time until battery " + "is fully charged",
      "Displays remaining battery charge in mAh", "Displays battery voltage in V", "Displays battery current in mA",
      "Displays battery temperature", "Displays the percent battery remaning", };
  ...
  private static final String[] powerShorts = { "Instant", "Minute Average", "Hour Average", "Day Average",
      "Total Average", "Battery Sensors", };
  private static final String[] powerLongs = { "Estimated instantaneous power consumption",
      "Average power consumption over the last minute", "Average power consumption over the last hour",
      "Average power consumption over the last day", "Average power consumption while the profiler has been running",
  ...
  private static final String[] chargeLongs = { "Use your battery's charge readings",
      "Assume that your battery is fully charged",
      "Assume that you have 1400 mAh worth of charge " + "(A typical battery capacity)", };
  ...
  private static final String[] tempLongs = { "Display battery temperature in celcius",
  ...
  private static final String[] battLongs = { "Display remaining life time when battery plugged in",
  ...
  public String[] getShortOptions(int level) {
    ...
    case ID_POWER:
      return powerShorts;
    case ID_BATTERY_TIME:
      return level == 1 ? powerShorts : (level == 2 ? chargeShorts : battShorts);
    case ID_TEMP:
      return tempShorts;
    }
    return null;
  ...
  public String[] getLongOptions(int level) {
    ...
    case ID_POWER:
      return powerLongs;
    case ID_BATTERY_TIME:
      return level == 1 ? powerLongs : (level == 2 ? chargeLongs : battLongs);
    case ID_TEMP:
      return tempLongs;
    }
    return null;
  ...
  public boolean hasOption(int level, int value) {
    ...
    if (level == 0) {
      if (value == ID_PERC)
        return bst.hasCapacity();
      if (value == ID_CHARGE)
        return bst.hasCharge();
      if (value == ID_CURRENT)
        return bst.hasCurrent();
      return true;
    }
    switch (id) {
    case ID_POWER:
    ...
    case ID_BATTERY_TIME:
      if (level == 1) {
        ...
        return value == CHARGE_1400 || value == CHARGE_SENSOR && bst.hasCharge() || value == CHARGE_FULL
            && bst.hasFullCapacity();
      }
      return true;
    case ID_TEMP:
  ...
  public String getValue(PowerEstimator p) {
    ...
    case ID_BATTERY_TIME: {
      if (bst.hasCurrent() && params[2] == BATT_CHARGETIME) {
        ...
        if (curr > 0 && cp >= 0.01) {
          ...
          return String.format("Charge time\n%1$d:%2$02d:%3$02d", time / 60 / 60, time / 60 % 60, time % 60);
        }
      }
    ...
    case ID_CHARGE: {
      return String.format("Charge\n%1$.1f mAh", calcCharge(CHARGE_SENSOR) / 3.6);
    }
    case ID_VOLTAGE: {
  ...
  private double calcPower(PowerEstimator p, int powId) {
    ...
    case POWER_INSTANT: {
      int count = 0;
      int[] history = p.getComponentHistory(5 * 60, -1, SystemInfo.AID_ALL, -1);
    ...
    case POWER_TOTAL: {
      int wind = 0;
      if (powId == POWER_MINUTE)
        wind = Counter.WINDOW_MINUTE;
      if (powId == POWER_HOUR)
        wind = Counter.WINDOW_HOUR;
      if (powId == POWER_DAY)
        wind = Counter.WINDOW_DAY;
      if (powId == POWER_TOTAL)
        wind = Counter.WINDOW_TOTAL;
      double total = 0;
