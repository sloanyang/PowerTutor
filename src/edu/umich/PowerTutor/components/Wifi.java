...
public class Wifi extends PowerComponent {
  public static class WifiData extends PowerData {
    ...
    public void init(double packets, long uplinkBytes, long downlinkBytes, double uplinkRate, double linkSpeed,
        int powerState) {
      wifiOn = true;
    ...
    public String getTextLogDataInfo() throws IOException {
      ...
      if (wifiOn) {
        res.append("Wifi-packets ").append((long) Math.round(packets)).append("\nWifi-uplinkBytes ")
            .append(uplinkBytes).append("\nWifi-downlinkBytes ").append(downlinkBytes).append("\nWifi-uplink ")
    ...
    public List<String> getCsvLogDataInfo() throws IOException {
      ...
      strings.add(wifiOn ? "on" : "off");
      if (wifiOn) {
        strings.add("" + (long) Math.round(packets));
        strings.add("" + uplinkBytes);
        strings.add("" + downlinkBytes);
        strings.add("" + uplinkRate);
        strings.add("" + linkSpeed);
        strings.add(Wifi.POWER_STATE_NAMES[powerState]);
      }
      return strings;
    }
  }
  ...
  public Wifi(Context context, PhoneConstants phoneConstants) {
    ...
    /*
     * Try to grab the interface name. If we can't find it will take a wild stab
     * in the dark.
     */
    ...
    transPacketsFile = "/sys/class/net/" + interfaceName + "/statistics/tx_packets";
    readPacketsFile = "/sys/class/net/" + interfaceName + "/statistics/rx_packets";
    transBytesFile = "/sys/class/net/" + interfaceName + "/statistics/tx_bytes";
    readBytesFile = "/sys/class/net/" + interfaceName + "/statistics/rx_bytes";
  ...
  public IterationData calculateIteration(long iteration) {
    ...
    if (wifiStateFlag != WifiManager.WIFI_STATE_ENABLED && wifiStateFlag != WifiManager.WIFI_STATE_DISABLING) {
      /*
    ...
    if (wifiState.isInitialized()) {
      ...
      WifiData data = WifiData.obtain();
      data.init(wifiState.getPackets(), wifiState.getUplinkBytes(), wifiState.getDownlinkBytes(),
          wifiState.getUplinkRate(), linkSpeed, wifiState.getPowerState());
      result.setPowerData(data);
    } else {
      wifiState.updateState(transmitPackets, receivePackets, transmitBytes, receiveBytes);
    }
      ...
      for (int uid : lastUids) {
        ...
        try {
          ...
          if (receiveBytes == -1 || transmitBytes == -1) {
            ...
            boolean active = transmitBytes != uidState.getTransmitBytes() || receiveBytes != uidState.getReceiveBytes();
            uidState.updateState(uidState.getTransmitPackets() + estimatedTransmitPackets, uidState.getReceivePackets()
            ...
            if (active) {
              ...
              uidData.init(uidState.getPackets(), uidState.getUplinkBytes(), uidState.getDownlinkBytes(),
                  uidState.getUplinkRate(), linkSpeed, uidState.getPowerState());
    ...
    public void updateState(long transmitPackets, long receivePackets, long transmitBytes, long receiveBytes) {
      ...
      if (lastTime != -1 && curTime > lastTime) {
        ...
        if (transmitPackets != lastTransmitPackets) {
          lastAverageTransmitPacketSize = 0.9 * lastAverageTransmitPacketSize + 0.1
              * (transmitBytes - lastTransmitBytes) / (transmitPackets - lastTransmitPackets);
        }
        if (receivePackets != lastReceivePackets) {
          lastAverageReceivePacketSize = 0.9 * lastAverageReceivePacketSize + 0.1 * (receiveBytes - lastReceiveBytes)
